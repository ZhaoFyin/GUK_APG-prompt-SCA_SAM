...
result_dirs = {"auto50": "./vis_results/auto",
               "auto100": "./vis_results/centroid",
               "auto200": "./vis_results/centroid_with_offset",}
...
for i, count in enumerate(["50", "100", "200"]):
    dataset = CustomDataset(voc_root=data_dir, kge=True, txt_name='tmp.txt', transform=transform, oh=True, sp=count)
    ...
    with (torch.no_grad()):
        ...
        for sampled_batch in loader:
            ...
            subplot_data[i][1] = diff_color(s_batch[0][0].detach().cpu().numpy())
            subplot_data[i][2] = prompts["mask"][0].detach().cpu().numpy()
            subplot_data[i][3] = overlap(img, pred, mask)
...
group_name = ["(d) 50地理单元下", "(e) 100地理单元下", "(f) 200地理单元下"]
axes_list = []
for h in range(h_c):
    row_axes = []
    for w in range(w_c):
...
plt.savefig(f'fig/Diff_count_result.png')
plt.close()