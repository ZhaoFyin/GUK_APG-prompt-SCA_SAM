...
import torchvision
from SAM.modeling_CNN import ImageEncoderViT, MaskDecoder, PromptEncoder
...
class Sam(nn.Module):
    ...
    def forward(self, batched_input, prompts, multimask_output, image_size):
        outputs = self.forward_train(batched_input, prompts, multimask_output, image_size)
        return outputs

    def forward_train(self, batched_input, prompts, multimask_output, image_size):
        # image_size = batched_input.shape[-1] # if test
        image_embeddings = self.image_encoder(batched_input)
        outputs = []
        point_pmt = prompts["point"]
        box_pmt = prompts["box"]
        mask_pmt = prompts["mask"]
        for point, box, mask, curr_embedding in zip(point_pmt, box_pmt, mask_pmt, image_embeddings):
            device = batched_input.device
            if point is not None:
                point_coords = torch.tensor([[[point[0].item(), point[1].item()]]], dtype=torch.float32)  # (1,1,2)
                point_labels = torch.tensor([[1]], dtype=torch.int64)  # (1,1)

                point = (point_coords.to(device), point_labels.to(device))
            if box is not None:
                box = torch.tensor(box,dtype=torch.float32).to(device).unsqueeze(0)
            if mask is not None:
        ...
        masks = torch.cat([output["masks"] for output in outputs], dim=0)
        iou_predictions = torch.cat([output["iou_predictions"] for output in outputs], dim=0)
        low_res_logits = torch.cat([output["low_res_logits"] for output in outputs], dim=0)
        outputs = {
                    "masks": masks,
                    "iou_predictions": iou_predictions,
